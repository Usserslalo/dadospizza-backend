// test-historial-pedidos.js
// Script de prueba para el endpoint de historial de pedidos
...
// Función para probar el endpoint de historial de pedidos
async function testHistorialPedidos() {
  try {
    console.log('🧪 Probando endpoint GET /api/orders/my-history...\n');
...
// Función para probar con token inválido
async function testTokenInvalido() {
  try {
    console.log('\n🧪 Probando endpoint con token inválido (debería fallar)...\n');
    ...
    console.log('❌ ERROR: El endpoint debería rechazar el token inválido');
    
  } catch (error) {
    if (error.response && error.response.status === 401) {
      console.log('✅ Correcto: El endpoint rechaza tokens inválidos');
    } else {
      console.error('❌ Error inesperado:', error.message);
    }
  }
